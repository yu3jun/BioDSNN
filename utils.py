...
def get_coeffs(singles_expr, first_expr, second_expr, double_expr):
    ...
    Args:
        singles_expr (np.array): single perturbation expression  # (454,2)
        first_expr (np.array): first perturbation expression   # (454,)
        second_expr (np.array): second perturbation expression  # (454,)
        double_expr (np.array): double perturbation expression  # (454,)

    """
    results = {}
    # 线性回归模型通过 c1 和 c2 的组合来预测 y。这可以帮助你理解不同扰动的交互作用如何影响最终的基因表达量
    results['ts'] = TheilSenRegressor(fit_intercept=False,
                          max_subpopulation=1e5,
                          max_iter=1000,
                          random_state=1000)
    
    X = singles_expr # (454,2) #  特征数据，454 个样本，每个样本有 2 个特征。这些特征将作为输入数据传递给回归模型
    y = double_expr # (454,)  # 目标变量（即你要预测的值）
    results['ts'].fit(X, y.ravel()) # np.ravel()将多维数组展平为一维数组，这里的fit用于训练模型。它接收特征数据 X 和目标数据 y
    Zts = results['ts'].predict(X)
    results['c1'] = results['ts'].coef_[0] # 第一个特征的回归系数：1.241142953597012，代表着第一种扰动对y的影响因子
    results['c2'] = results['ts'].coef_[1] # 第二个特征的回归系数：0.264259840071161，代表着第二种扰动对y的影响因子
    results['mag'] = np.sqrt((results['c1']**2 + results['c2']**2))  # 回归系数向量的模长：1.2689637876385402
    # 模长提供了系数向量的整体规模信息。如果模长很小，可能表明这两个扰动组合后对基因表达量的影响都不强，
    # 如果 c1 和 c2 都很大，那么模长也会很大，表示两个扰动共同作用对基因表达的影响较大
...
def get_GI_params(preds, combo):
    ...
    """
    singles_expr = np.array([preds[combo[0]], preds[combo[1]]]).T  # 转置之前shape:(2,454)， 转置之后shape:(454, 2)
    first_expr = np.array(preds[combo[0]]).T # shape:(454,)
    second_expr = np.array(preds[combo[1]]).T # shape:(454,)
    # double_expr 会从preds找出双扰动 'pert1_pert2'对应的表达值
    double_expr = np.array(preds[combo[0]+'_'+combo[1]]).T  # shape:(454,)
